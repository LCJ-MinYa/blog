...
有一个表单，根据某个属性分为A,B两种状态，A状态中包含（label: 名称A, prop: 名称），B状态中包含（label: 名称B, prop: 名称），A名称和B名称所使用的prop名称一样，导致在提交form校验的时候，之前A状态可以正常校验，B状态中包含重复prop名称的都无法校验，这就是没有key，默认用prop当作了唯一标识，导致无法检测更改，通过以下更改达到目的。
```vue
<el-form-item label="名称A" prop="名称" key="AKey"></el-form-item>
<el-form-item label="名称B" prop="名称" key="BKey"></el-form-item>
```

启发：凡是渲染无法生效，无法赋值等，优先考虑key问题，同时，key是对所有dom元素都是生效的，并不仅仅是在v-for中使用。
{% endnote %}
